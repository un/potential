 ...
 */
export const ImagePickerUploader: React.FC<ImagePickerUploaderProps> = ({
  onImagesUploaded,
  maxImages = 10,
  metadata: _metadata = {},
  ...
  const [uploadedImages, setUploadedImages] = useState<ExtendedUploadedImage[]>(
    [],
  );
  ...
  const removeUploadedImage = (index: number) => {
    ...
    if (onImagesUploaded) {
      const baseUploadedImages: UploadedImage[] = newUploadedImages.map(
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
        ({ imageId }) => ({ imageId }),
      );
      onImagesUploaded(baseUploadedImages);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (onImagesUploaded) {
        const baseUploadedImages: UploadedImage[] = updatedImages.map(
          // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
          ({ imageId }) => ({ imageId }),
        );
        onImagesUploaded(baseUploadedImages);
